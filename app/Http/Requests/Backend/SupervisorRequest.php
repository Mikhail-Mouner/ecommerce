...
class SupervisorRequest extends FormRequest
{
    ...
    public function rules()
    {
        switch ($this->method()) {
            ...
            {
                return [
                    'username' => 'required|string|unique:App\Models\User,username,' . $this->route()->supervisor->id . '|min:3|max:191',
                    'first_name' => 'required|string|min:3|max:191',
                    'last_name' => 'required|string|min:3|max:191',
                    'email' => 'required|email|unique:App\Models\User,email,' . $this->route()->supervisor->id,
                    'phone' => 'required|string|unique:App\Models\User,phone,' . $this->route()->supervisor->id,
                    'status' => 'required|numeric|boolean',
