                            ...
                            <div wire:ignore class="p-5 my-md-4">
                                <ul class="list-inline mb-2">
                                    @if($productModal->reviews_avg_rating != '')
                                        {{ round($productModal->reviews_avg_rating) }}
                                        @for($i=1;$i<=$productModal->reviews_avg_rating;$i++)
                                            <li class="list-inline-item m-0"><i
                                                        class="fas fa-star fa-sm text-warning"></i>
                                            </li>
                                        @endfor
                                        @for($i=$productModal->reviews_avg_rating;$i<5;$i++)
                                ...
                                <p class="text-muted">{{ "{$productModal->price} EGP" }}</p>
                                ...
                                <div class="row align-items-stretch mb-4">
                                    <div class="col-sm-7 pr-sm-0">
                                        <div class="border d-flex align-items-center justify-content-between py-1 px-3">
                                            ...
                                            <div class="quantity">
                                                <button wire:click="decreaseQuantity()" class="dec-btn p-0"><i
                                                            class="fas fa-caret-left"></i></button>
                                                <input wire:model="qty" class="form-control border-0 shadow-0 p-0"
                                                       type="text" value="1">
                                                <button wire:click="increaseQuantity()" class="inc-btn p-0"><i
