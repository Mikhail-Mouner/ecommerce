<tr x-data="{ show: true }" x-show="show">
    ...
    <td class="align-middle border-0">
        <div class="border d-flex align-items-center justify-content-between px-3">
            ...
            <div class="quantity">
                <button wire:click.prevent="decreaseQuantity()" class="dec-btn p-0"><i class="fas fa-caret-left"></i>
                </button>
                <span class="form-control form-control-sm border-0 shadow-0 px-2 text-center">{{ $item_qty }}</span>
                {{--<input class="form-control form-control-sm border-0 shadow-0 p-0"
                       type="text"
                       value="" />--}}
                <button wire:click.prevent="increaseQuantity()" class="inc-btn p-0"><i class="fas fa-caret-right"></i>
                </button>
            </div>
        </div>
    </td>
    <td class="align-middle border-0">
        <p class="mb-0 small">{{ "{$cart_item->subtotal} EGP" }}</p>
    </td>
    <td class="align-middle border-0">
